...
class WtMailNotifier:
    ...
    def __init__(self, user: str, pwd: str, sender: str = None, host: str = "smtp.exmail.qq.com", port=465,
                 isSSL: bool = True):
        self.user = user
        self.pwd = pwd
...
class WtHotPicker:
            ...
            if items is not None:
                ...
                for key in hots.keys():
                    ...
                    if key not in lastHots:
                        ...
                        logging.info("[%s]品种%s主力确认, 确认日期: %s, %s", exchg, key, nextDT.strftime('%Y%m%d'),
                                     hots[key])
                    else:
                        ...
                        if oldItem is None or newItem.month > oldItem.month:
                            ...
                            logging.info("[%s]品种%s主力切换 切换日期: %s，%s -> %s", exchg, key,
                                         nextDT.strftime('%Y%m%d'), lastHots[key], hots[key])
                            lastHots[key] = hots[key]
                ...
                for key in seconds.keys():
                    ...
                    if key not in lastSecs:
                        ...
                        logging.info("[%s]品种%s次主力确认, 确认日期: %s, %s", exchg, key, nextDT.strftime('%Y%m%d'),
                                     seconds[key])
                    else:
                        ...
                        if oldItem is None or newItem.month > oldItem.month:
                            ...
                            logging.info("[%s]品种%s次主力切换 切换日期: %s，%s -> %s", exchg, key,
                                         nextDT.strftime('%Y%m%d'), lastSecs[key], seconds[key])
                            lastSecs[key] = seconds[key]
            # 日期递增
    ...
    def execute_rebuild(self, beginDate: datetime.datetime = None, endDate: datetime.datetime = None,
                        exchanges=["CFFEX", "SHFE", "CZCE", "DCE", "INE"], wait=False):
        """
        重构全部的主力切换规则
        ...
        if self.mail_notifier is not None:
            self.mail_notifier.notify(hot_changes,
                                      sec_changes,
                                      endDate,
                                      hotFile,
                                      "hotmap.json",
    ...
    def execute_increment(self, endDate: datetime.datetime = None, exchanges=["CFFEX", "SHFE", "CZCE", "DCE", "INE"]):
        ...
        if bChanged:
            ...
            if self.mail_notifier is not None:
                self.mail_notifier.notify(hot_changes, sec_changes, endDate, hotFile, "hotmap.json", secFile,
                                          "secmap.json")
        else:
