...
def performance_summary(input_data, input_data1, capital=500000, rf=0.00, period=240, data2=[]):
    ...
    input_data1['adjust_profit'] = (input_data1['profit'] - input_data1['transaction_fee']) if len(
        input_data1) > 0 else 0
    # 调整毛利
...
def trading_analyze(workbook: Workbook, df_closes, df_funds, capital=500000):
    ...
    worksheet.write_row('A%d' % (next_row + 3), ['期间', '盈利(¤)', '盈利(%)', '毛利', '毛损', '交易次数', '胜率(%)'],
                        index_format)
...
def strategy_analyze(workbook: Workbook, df_closes, df_trades, df_funds, capital, rf=0.0, period=240):
    ...
    df_temp['profit'] = df_closes[df_closes['direct'] == 'LONG']['profit'] - df_closes[df_closes['direct'] == 'LONG'][
        'fee']
    ...
    short_df = df_closes[df_closes['direct'] == 'SHORT']
    df_temp2['profit'] = short_df['short_profit'] - short_df['fee']
...
def output_closes(workbook: Workbook, df_closes: df, capital=500000):
    ...
    worksheet.write_row('A3',
                        ['编号', '代码', '方向', '进场时间', '进场价格', '进场标记', '出场时间', '出场价格', '出场标记',
                         '盈利¤', '盈利%', '累计盈利¤', '累计盈利%', '潜在盈利¤', '潜在盈利%', '潜在亏损¤', '潜在亏损%',
                         '累计权益', '平仓K线编号'], index_format)
    ...
    ay = df_closes['entertag'].apply(lambda x: x if isinstance(x, str) else '' if math.isnan(x) else x)
    ...
    ay = df_closes['exittag'].apply(lambda x: x if isinstance(x, str) else '' if math.isnan(x) else x)
...
def summary_analyze(df_funds: df, capital=5000000, rf=0, period=240) -> dict:
    ...
    down_delta = fmtNAN(ayDailyReturn[ayDailyReturn < 0].std(axis=0) * math.pow(annual_days, 0.5),
                        0)  # 下行标准差=每日亏损收益率标准差*根号下(年交易日天数)
...
def funds_analyze(workbook: Workbook, df_funds: df, capital=5000000, rf=0, period=240):
    ...
    key_indicator = ['交易天数', '累积收益（%）', '年化收益率（%）', '胜率（%）', '最大回撤（%）', '最大上涨（%）', '标准差（%）',
                     '下行波动率（%）', 'Sharpe比率', 'Sortino比率', 'Calmar比率']
    key_data = [(ayNetVals.iloc[-1] - 1) * 100, ar * 100, (windays / days) * 100, mdd * 100, mup * 100, delta * 100,
                down_delta * 100, sr, sortino, calmar]
...
class WtBtAnalyst:
    ...
    def add_strategy(self, sname: str, folder: str, init_capital: float, rf: float = 0.02,
    ...
    def run_new(self, outFileName: str = ''):
        ...
        for sname in self.__strategies__:
            ...
            if len(outFileName) == 0:
                outFileName = 'Strategy[%s]_PnLAnalyzing_%s_%s.xlsx' % (
                    sname, df_funds['date'][0], df_funds['date'].iloc[-1])
            ...
            strategy_analyze(workbook, df_closes.copy(), df_trades.copy(), df_funds.copy(), capital=init_capital, rf=rf,
                             period=annual_days)
