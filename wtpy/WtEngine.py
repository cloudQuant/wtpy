...
class WtEngine:
    ...
    def __init__(self, eType: EngineType, logCfg: str = "logcfg.yaml", genDir: str = "generated",
                 bDumpCfg: bool = False):
        """
        WtEngine构造函数
    ...
    def add_exetended_parser(self, parser: BaseExtParser):
        ...
        if ids not in self.__ext_parsers__:
            if self.__wrapper__.create_extended_parser(ids):
                self.__ext_parsers__[ids] = parser

    def add_exetended_executer(self, executer: BaseExtExecuter):
        ids = executer.id()
        if ids not in self.__ext_executers__:
            if self.__wrapper__.create_extended_executer(ids):
                self.__ext_executers__[ids] = executer
    ...
    def get_extended_executer(self, ids: str) -> BaseExtExecuter:
        if ids not in self.__ext_executers__:
            return None
        return self.__ext_executers__[ids]
    ...
    def write_indicator(self, ids: str, tag: str, time: int, data: dict):
        """
        写入指标数据
        """
        if self.__writer__ is not None:
            self.__writer__.write_indicator(ids, tag, time, data)
    ...
    def addExternalCtaStrategy(self, ids: str, params: dict):
        """
        添加外部的CTA策略
        """
    ...
    def addExternalHftStrategy(self, ids: str, params: dict):
        """
        添加外部的HFT策略
        """
        if "strategies" not in self.__config__:
    ...
    def addExecuter(self, ids: str, trader: str, policies: dict, scale: int = 1):
    ...
    def addTrader(self, ids: str, params: dict):
    ...
    def add_cta_strategy(self, strategy: BaseCtaStrategy):
        ...
        @strategy   策略对象
        """
        ids = self.__wrapper__.create_cta_context(strategy.name())
        self.__cta_ctxs__[ids] = CtaContext(ids, strategy, self.__wrapper__, self)
    ...
    def add_hft_strategy(self, strategy: BaseHftStrategy, trader: str, agent: bool = True):
        ...
        @strategy   策略对象
        """
        ids = self.__wrapper__.create_hft_context(strategy.name(), trader, agent)
        self.__hft_ctxs__[ids] = HftContext(ids, strategy, self.__wrapper__, self)

    def add_sel_strategy(self, strategy: BaseSelStrategy, date: int, time: int, period: str):
        ids = self.__wrapper__.create_sel_context(strategy.name(), date, time, period)
        self.__sel_ctxs__[ids] = SelContext(ids, strategy, self.__wrapper__, self)

    def get_context(self, ids: int):
        """
        根据ID获取策略上下文
        @id     上下文id，一般添加策略的时候会自动生成一个唯一的上下文id
        """
        if self.__engine_type == EngineType.ET_CTA:
            if ids not in self.__cta_ctxs__:
                return None

            return self.__cta_ctxs__[ids]
        elif self.__engine_type == EngineType.ET_HFT:
            if ids not in self.__hft_ctxs__:
                return None

            return self.__hft_ctxs__[ids]
        elif self.__engine_type == EngineType.ET_SEL:
            if ids not in self.__sel_ctxs__:
                return None

            return self.__sel_ctxs__[ids]
