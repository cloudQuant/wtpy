...
class WtMonSvr(WatcherSink):
    ...
    def init_mgr_apis(self, app: Flask):
            ...
            if True:
                ...
                if len(user) == 0 or len(pwd) == 0:
                    ...
                    if usrInf is None:
                        ...
                        session["expiretime"] = exptime.replace(tzinfo=pytz.timezone('UTC')).strftime(
                            "%Y.%m.%d %H:%M:%S")
        ...
        def cmd_del_app():
            ...
            if len(ids) == 0:
            ...
            elif self.__data_mgr__.has_group(ids):
                ret = {
                    "result": -2,
                    "message": "该调度任务是策略组合，请从组合管理删除"
                }
            elif not self._dog.has_app(ids):
                ret = {
                    "result": -3,
                    "message": "该调度任务不存在"
                }
            elif self._dog.isRunning(ids):
                ret = {
                    "result": -4,
                    "message": "请先停止该任务"
                }
                ...
                if True:
                    self._dog.delApp(ids)
                    ret = {
                        "result": 0,
                        "message": "Ok"
                    }

                    self.__data_mgr__.log_action(adminInfo, "delapp", ids)
                else:
                    ret = {
                        "result": -1,
                        "message": "请求解析失败"
        ...
        def cmd_commit_group_filters():
            ...
            grpid = get_param(json_data, "groupid")
            filters = get_param(json_data, "filters", func=dict)
            if not self.__data_mgr__.has_group(grpid):
                ret = {
                    "result": -1,
                    "message": "组合不存在"
