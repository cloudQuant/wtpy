...
class WtDataHelper:
    ...
    def __init__(self):
        ...
        self.api.resample_bars.argtypes = [c_char_p, CB_DTHELPER_BAR, CB_DTHELPER_COUNT, c_uint64, c_uint64, c_char_p,
                                           c_uint, c_char_p, CB_DTHELPER_LOG]
    ...
    def dump_bars(self, binFolder: str, csvFolder: str, strFilter: str = ""):
        ...
        """
        self.api.dump_bars(bytes(binFolder, encoding="utf8"), bytes(csvFolder, encoding="utf8"),
                           bytes(strFilter, encoding="utf8"), self.cb_dthelper_log)
    ...
    def trans_csv_bars(self, csvFolder: str, binFolder: str, period: str):
        ...
        """
        self.api.trans_csv_bars(bytes(csvFolder, encoding="utf8"), bytes(binFolder, encoding="utf8"),
                                bytes(period, encoding="utf8"), self.cb_dthelper_log)
    ...
    def read_dsb_ticks(self, tickFile: str) -> TickList:
        ...
        """
        tick_cache = TickList()
        if 0 == self.api.read_dsb_ticks(bytes(tickFile, encoding="utf8"), CB_DTHELPER_TICK(tick_cache.on_read_tick),
                                        CB_DTHELPER_COUNT(tick_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return tick_cache
    ...
    def read_dsb_bars(self, barFile: str) -> BarList:
        ...
        """
        bar_cache = BarList()
        if 0 == self.api.read_dsb_bars(bytes(barFile, encoding="utf8"), CB_DTHELPER_BAR(bar_cache.on_read_bar),
                                       CB_DTHELPER_COUNT(bar_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return bar_cache
    ...
    def read_dmb_ticks(self, tickFile: str) -> TickList:
        ...
        """
        tick_cache = TickList()
        if 0 == self.api.read_dmb_ticks(bytes(tickFile, encoding="utf8"), CB_DTHELPER_TICK(tick_cache.on_read_tick),
                                        CB_DTHELPER_COUNT(tick_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return tick_cache
    ...
    def read_dmb_bars(self, barFile: str) -> BarList:
        ...
        """
        bar_cache = BarList()
        if 0 == self.api.read_dmb_bars(bytes(barFile, encoding="utf8"), CB_DTHELPER_BAR(bar_cache.on_read_bar),
                                       CB_DTHELPER_COUNT(bar_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return bar_cache
    ...
    def store_bars(self, barFile: str, firstBar: POINTER(WTSBarStruct), count: int, period: str) -> bool:
        ...
        @period     周期，m1/m5/d
        """
        return self.api.store_bars(bytes(barFile, encoding="utf8"), firstBar, count, bytes(period, encoding="utf8"),
                                   self.cb_dthelper_log)
    ...
    def resample_bars(self, barFile: str, period: str, times: int, fromTime: int, endTime: int,
                      sessInfo: SessionInfo) -> BarList:
        """
        重采样K线
        ...
        """
        bar_cache = BarList()
        if 0 == self.api.resample_bars(bytes(barFile, encoding="utf8"), CB_DTHELPER_BAR(bar_cache.on_read_bar),
                                       CB_DTHELPER_COUNT(bar_cache.on_data_count),
                                       fromTime, endTime, bytes(period, 'utf8'), times,
                                       bytes(sessInfo.toString(), 'utf8'), self.cb_dthelper_log):
            return None
        else:
            return bar_cache
