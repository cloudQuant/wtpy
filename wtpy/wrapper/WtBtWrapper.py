...
class WtBtWrapper:
    ...
    def on_stra_calc_done(self, ids: int, curDate: int, curTime: int):
        engine = self._engine
        ctx = engine.get_context(ids)
        if ctx is not None:
    ...
    def on_stra_get_position(self, ids: int, stdCode: str, qty: float, isLast: bool):
        engine = self._engine
        ctx = engine.get_context(ids)
        if ctx is not None:
    ...
    def initialize_hft(self, logCfg: str = "logcfgbt.yaml", isFile: bool = True, outDir: str = "./outputs_bt"):
        ...
        try:
            ...
            self.api.register_hft_callbacks(self.cb_stra_init, self.cb_stra_tick, self.cb_stra_bar,
                                            ...
                                            self.cb_hftstra_entrust, self.cb_hftstra_order_detail,
                                            self.cb_hftstra_order_queue,
    ...
    def register_extended_data_loader(self, bAutoTrans: bool = True):
        ...
        self.api.register_ext_data_loader(self.cb_load_fnlbars, self.cb_load_rawbars, self.cb_load_adjfacts,
                                          self.cb_load_histicks, bAutoTrans)
    ...
    def cta_step(self, ids: int) -> bool:
        """
        单步执行
        @id         策略id
        """
        return self.api.cta_step(ids)
    ...
    def hft_get_ordque(self, ids: int, stdCode: str, count: int):
        ...
        return self.api.hft_get_ordque(ids, bytes(stdCode, encoding="utf8"), count,
                                       CB_HFTSTRA_GET_ORDQUE(self.on_hftstra_order_queue))
    ...
    def hft_get_orddtl(self, ids: int, stdCode: str, count: int):
        ...
        return self.api.hft_get_orddtl(ids, bytes(stdCode, encoding="utf8"), count,
                                       CB_HFTSTRA_GET_ORDDTL(self.on_hftstra_order_queue))
    ...
    def hft_get_trans(self, ids: int, stdCode: str, count: int):
        ...
        return self.api.hft_get_trans(ids, bytes(stdCode, encoding="utf8"), count,
                                      CB_HFTSTRA_GET_TRANS(self.on_hftstra_order_queue))
    ...
    def hft_step(self, ids: int):
        """
        单步执行
        @id         策略id
        """
        self.api.hft_step(ids)
    ...
    def init_sel_mocker(self, name: str, date: int, time: int, period: str, trdtpl: str = "CHINA",
                        session: str = "TRADING", slippage: int = 0) -> int:
        """
        创建策略环境
        @name      策略名称
        @return    系统内策略ID 
        ...
        return self.api.init_sel_mocker(bytes(name, encoding="utf8"), date, time,
                                        ...
                                        bytes(session, encoding="utf8"), slippage)
